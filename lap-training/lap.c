...
#define CANDIDATE_POOL_SIZE 32 // Gruppo iniziale sufficiente per garantire 8 collisioni

// Funzione ausiliaria: verifica se l'INTERO gruppo 'group' caccia via 'target_ptr'
static int test_group_evicts_target(void *target_ptr, uint8_t **group, int group_size) {
    int evicts = 0;
    ...
    return (evicts >= 6); // Ritorna vero se espelle con successo almeno 6 volte su 8
}
...
void find_eviction_set(void *target_ptr) {
    ...
    // FASE 1: Raccogli 32 candidati che hanno gli stessi bit di offset (Bit 7..11)
    for (size_t i = 0; i < total_lines && cand_count < CANDIDATE_POOL_SIZE; i++) {
        uint8_t *cand = &probe_buf[i * CACHELINE_SIZE];
        if (cand == target_ptr) continue;
    ...
    if (cand_count < CANDIDATE_POOL_SIZE) {
        printf("[-] Errore: Trovati solo %d candidati con offset compatibile.\n", cand_count);
        exit(1);
    }
    ...
    if (!test_group_evicts_target(target_ptr, candidates, cand_count)) {
        printf("[-] Errore: Il gruppo da 32 non riesce a svuotare il cassetto L1D!\n");
        exit(1);
    }

    printf("[+] Gruppo iniziale valido. Inizio sfoltimento da 32 a 8 linee...\n");

    // FASE 2: Sfoltimento (Group Reduction) - Rimuovi le linee inutili una ad una
    int current_size = cand_count;
    
    for (int i = 0; i < current_size && current_size > EVICTION_SIZE; i++) {
        uint8_t *temp = candidates[i];
        candidates[i] = NULL; // Togliamo temporaneamente la linea i-esima dal sacco
        ...
        if (test_group_evicts_target(target_ptr, candidates, current_size)) {
            ...
            // ...se il target NON cade più, la linea era FONDAMENTALE. La rimettiamo!
            candidates[i] = temp;
        }
    }
