...
int main(int argc, char *argv[]) {
    ...
    //for (int ITERS = MIN_ITERS; ITERS <= MAX_ITERS; ITERS += ITERS_STEP) {
        ...
        void *buffer =
            mmap(NULL, NUM_PAGES_BUF * PAGE_SZ, PROT_READ | PROT_WRITE,
        ...
        // definisci il valore massimo che possiamo scrivere in maniera tale
        // che, se usato come indice, il risultato non sarò out of bounds
        ...
        // dry run per portare gli indirizzi in cache, dimostrando che lo
        // speedup non è dovuto al prefetching in cache
        junk = 0;
